...
def main(args):
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(args, model, train_dataloader, optimizer, lr_scheduler, epoch, model_without_ddp=model_without_ddp)
...
def train_one_epoch(args, model, data_loader, optimizer, lr_scheduler, epoch, model_without_ddp):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.train()
    ...
    for step, (src_input, tgt_input) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        lr_scheduler.step()  # aggiorna LR
        print(f"[DEBUG] Epoch {epoch} - Step {step} - LR: {lr_scheduler.get_last_lr()[0]:.8f}")
...
def evaluate(args, data_loader, model, model_without_ddp):
    ...
    # Filtro predizioni vuote (e relativi target)
    filtered_pres_refs = [(p, r) for p, r in zip(tgt_pres, tgt_refs) if p.strip() != ""]
    if not filtered_pres_refs:
        print("[WARNING] Tutte le ipotesi sono vuote, salto il calcolo delle metriche.")
        return {"bleu4": 0.0, "rouge": 0.0, "loss": 0.0}

    filtered_pres, filtered_refs = zip(*filtered_pres_refs)

    bleu_dict, rouge_score = translation_performance(filtered_refs, filtered_pres)
