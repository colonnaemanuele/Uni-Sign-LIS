...
def bbox_4hands(left_keypoints, right_keypoints, hw):
    ...
    right_mean_x, right_mean_y, right_diff_x, right_diff_y = compute_bbox(
        right_keypoints
    )
    ...
    left_new_box = np.stack(
        [
            left_mean_x - box_hw / 2,
            left_mean_y - box_hw / 2,
            left_mean_x + box_hw / 2,
            left_mean_y + box_hw / 2,
        ]
    ).astype(np.int16)
    right_new_box = np.stack(
        [
            right_mean_x - box_hw / 2,
            right_mean_y - box_hw / 2,
            right_mean_x + box_hw / 2,
            right_mean_y + box_hw / 2,
        ]
    ).astype(np.int16)
...
def load_support_rgb_dict(tmp, skeletons, confs, full_path, data_transform):
    ...
    #  21 keypoint della mano sinistra e  calcola la confidence media per ciascun frame
    left_confs_filter = confs[:, 0, 91:112].mean(-1)  
    # restituisce gli indici dei frame con mano sinistra visibile
    ...
    if len(left_confs_filter_indices) == 0:
        ...
        left_sampled_indices = np.random.choice(
            left_confs_filter_indices.tolist(),
    ...
    if len(right_confs_filter_indices) == 0:
        ...
        right_confs = right_confs[
            :, 0, [95 + 21, 99 + 21, 103 + 21, 107 + 21, 111 + 21]
        ].min(-1)
        ...
        right_sampled_indices = np.random.choice(
            right_confs_filter_indices.tolist(),
    ...
    # get hand bbox
    left_new_box, right_new_box, box_hw = bbox_4hands(
        left_skeletons, right_skeletons, imgs[0].shape[:2]
    )
    ...
    if left_sampled_indices is None:
        ...
        left_skeletons_norm = (
            left_skeletons * imgs[0].shape[:2][::-1] - left_new_box[:, None, [0, 1]]
        )
    ...
    if right_sampled_indices is None:
        ...
        right_skeletons_norm = (
            right_skeletons * imgs[0].shape[:2][::-1] - right_new_box[:, None, [0, 1]]
        )
    ...
    for idx, img in enumerate(imgs):
        ...
        if (
            left_sampled_indices is not None
            and left_idx < len(left_sampled_indices)
            and mapping_idx == left_sampled_indices[left_idx]
        ):
            ...
            img_draw = np.pad(
                img_draw,
                (
                    (0, max(0, box_hw - img_draw.shape[0])),
                    (0, max(0, box_hw - img_draw.shape[1])),
                    (0, 0),
                ),
                mode="constant",
                constant_values=0,
        ...
        if (
            right_sampled_indices is not None
            and right_idx < len(right_sampled_indices)
            and mapping_idx == right_sampled_indices[right_idx]
        ):
    ...
    def collate_fn(self, batch):
        ...
        if self.rgb_support:
            ...
            for part in ["left", "right"]:
                ...
                src_input[len_key] = [
                    len(index) for index in support_rgb_dicts[index_key]
                ]
...
class LIS_Dataset(Base_Dataset):
    ...
    def load_pose(self, pose_name, rgb_name):
        ...
        pose["keypoints"] = [
            np.array(k) if not isinstance(k, np.ndarray) else k
            for k in pose["keypoints"]
        ]
        pose["scores"] = [
            np.array(s) if not isinstance(s, np.ndarray) else s for s in pose["scores"]
        ]
        #########################################################################################################
        ...
        if max_val > 10:
            pose["keypoints"] = [
                np.array(kp) / [frame_w, frame_h] for kp in pose["keypoints"]
            ]
        ...
        if self.rgb_support:
            ...
            support_rgb_dict = load_support_rgb_dict(
                tmp, skeletons, confs, full_path, self.transform
            )
...
class LIS_Dataset_online(Base_Dataset):
    ...
    def load_pose(self):
        ...
        if self.rgb_support:
            ...
            support_rgb_dict = load_support_rgb_dict(
                tmp, skeletons, confs, full_path, self.data_transform
            )
